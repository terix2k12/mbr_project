...
import jcolibri.method.retrieve.Footprint.FPScoringMethod;
...
/**
 * Test 1 shows how to use a simple data base connector and perform the KNN
 * retrieval. It uses the travel example with cases that only have description
 * (without solution or result). <br>
 * This example uses the DataBase connector that is implemented using the <a
 * href="www.hibernate.org">Hibernate</a> library. That library is a Java Data
 * Objects implementation that automatically manages the persistence of Java
 * beans in relational data bases. <br>
 * For an introduction to hibernate see: <a
 * href="http://www.hibernate.org/hib_docs/v3/reference/en/html/tutorial.html"
 * >http://www.hibernate.org/hib_docs/v3/reference/en/html/tutorial.html</a>
 * <p>
 * The DataBase connector in jcolibri/test/test1/databaseconfig.xml and that xml
 * defines the configuration of Hibernate and the mapping of the description of
 * our case with the data base. In this example, all the attributes of the
 * description bean are stored in the same table. <br>
 * <ul>
 * <li><b>databaseconfig.xml</b>.<br>
 * Connector configuration file: indicates the path to the hibernate config
 * file, the name of the class/bean that stores the description and its mapping
 * file.<br>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * <li><b>hibernate.cfg.xml</b>.<br>
 * Hibernate configuration file: developers should only modify the database
 ...
 * If you use another database change the driver, url and dialect fields. For
 * example, to use a MySQL server you should use:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * For other configuration settings see hiberante documentation: <a href="http://www.hibernate.org/hib_docs/v3/reference/en/html/session-configuration.html"
 * >http://www.hibernate.org/hib_docs/v3/reference/en/html/session-configuration
 * .html</a>
 * </p>
 * </li>
 * <li><b>TravelDescription.hbm.xml</b>.<br>
 * Hibernate mapping file for the description bean the case
 * (TravelDescription.java). It stores each attribute of the description in a
 * column of the table Travel.<br>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * Here we set that TravelDescription is mapped in the Travel table. caseId is
 * the primary key of the table and Hibernate will use a native key generator
 * for new cases (there are different ways to create primary keys, for more
 * information see Hiberante documentation). Each attribute is mapped into a
 * table with the same name. You should notice that here we don't indicate the
 * type of the attributes. Hibernate automatically detects the type and converts
 * from/to the database. Anyway if you want to use an unrecognoized type that
 * hibernate does not understand or create your own one you can do it by
 * implementing the jcolibri.connector.TypeAdaptor interface in your type. <br>
 ...
 */
public class TestFP implements StandardCBRApplication {

	private static GUIInterface maininterface;
	/** Connector object */
	...
	public void configure() throws ExecutionException {
		try {
			...
			_connector.initFromXMLfile(jcolibri.util.FileIO
					.findFile("jcolibri/test/test1/databaseconfig.xml"));
	...
	public void cycle(CBRQuery query) throws ExecutionException {
		...
		Collection<CBRCase> eval = FPScoringMethod.retrieveCases(query,
				simConfig);
		// Print the retrieval
	...
	public static void main(String[] args) {
		...
		TestFP test1 = new TestFP();
		try {
			maininterface = new GUIInterface(test1);
